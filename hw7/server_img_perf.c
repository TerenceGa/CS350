...
struct connection_params {
	...
	uint64_t perf_type;   // Added for hardware event type
    uint64_t perf_config; // Added for hardware event config
};
		    ...
		    struct worker_params * common_params)
{
	...
	if (cmd == WORKERS_START) {
		...
		for (i = 0; i < worker_count; ++i) {
            ...
            worker_params[i]->perf_type = common_params->perf_type;   // Set perf_type
            worker_params[i]->perf_config = common_params->perf_config; // Set perf_config
...
void handle_connection(int conn_socket, struct connection_params conn_params)
{
	...
	// Initialize common_worker_params with perf_type and perf_config
	common_worker_params.conn_socket = conn_socket;
	common_worker_params.the_queue = the_queue;
	common_worker_params.worker_done = 0;
	common_worker_params.worker_id = 0; // Since only 1 worker is supported
	common_worker_params.perf_type = conn_params.perf_type;
	common_worker_params.perf_config = conn_params.perf_config;
	...
	while((opt = getopt(argc, argv, "q:w:p:h:")) != -1) {
		...
		case 'h':
            if (!strcmp(optarg, "INSTR")) {
                conn_params.perf_type = PERF_TYPE_HARDWARE;
                conn_params.perf_config = PERF_COUNT_HW_INSTRUCTIONS;
                printf("INFO: profiling INSTR (CPU instructions)\n");
            }
            else if (!strcmp(optarg, "L1MISS")) {
                conn_params.perf_type = PERF_TYPE_HW_CACHE;
                conn_params.perf_config = (PERF_COUNT_HW_CACHE_L1D) | 
                                           (PERF_COUNT_HW_CACHE_OP_READ << 8) |
                                           (PERF_COUNT_HW_CACHE_RESULT_MISS << 16);
                printf("INFO: profiling L1 cache misses\n");
            }
            else if (!strcmp(optarg, "LLCMISS")) {
                conn_params.perf_type = PERF_TYPE_HW_CACHE;
                conn_params.perf_config = (PERF_COUNT_HW_CACHE_LL) | 
                                           (PERF_COUNT_HW_CACHE_OP_READ << 8) |
                                           (PERF_COUNT_HW_CACHE_RESULT_MISS << 16);
                printf("INFO: profiling LLC cache misses\n");
            }
		default: /* '?' */
