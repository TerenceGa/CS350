...
int add_to_queue(struct request_meta to_add, struct queue *the_queue) {
    ...
    if (the_queue->available == 0) {
        ...
        if (the_queue->policy == QUEUE_FIFO) {
            ...
            // SJN policy: Insert in order of request length

            size_t num_items = the_queue->max_size - the_queue->available;

            struct request_meta *temp_requests = (struct request_meta *)malloc(sizeof(struct request_meta) * (num_items + 1));

            for (size_t i = 0; i < num_items; i++) {
                size_t idx = (the_queue->rd_pos + i) % the_queue->max_size;
                temp_requests[i] = the_queue->requests[idx];
            }

            size_t insert_idx = 0;
            while (insert_idx < num_items &&
                   TSPEC_TO_DOUBLE(temp_requests[insert_idx].request.req_length) <=
                   TSPEC_TO_DOUBLE(to_add.request.req_length)) {
                insert_idx++;
            }

            for (size_t i = num_items; i > insert_idx; i--) {
                temp_requests[i] = temp_requests[i - 1];
            }
            temp_requests[insert_idx] = to_add;

            for (size_t i = 0; i < num_items + 1; i++) {
                size_t idx = (the_queue->rd_pos + i) % the_queue->max_size;
                the_queue->requests[idx] = temp_requests[i];
            }

            the_queue->wr_pos = (the_queue->rd_pos + num_items + 1) % the_queue->max_size;
            the_queue->available--;
...
int main (int argc, char ** argv) {
	...
	while((opt = getopt(argc, argv, "q:w:")) != -1) {
        ...
        if (strcmp(optarg, "FIFO") == 0) {
            ...
            printf("INFO: setting policy = FIFO\n");
        } else if (strcmp(optarg, "SJN") == 0) {
            conn_params.policy = QUEUE_SJN;
            printf("INFO: setting policy = SJN\n");
        } else {
            fprintf(stderr, "Invalid policy '%s'. Valid options are FIFO or SJN.\n", optarg);
